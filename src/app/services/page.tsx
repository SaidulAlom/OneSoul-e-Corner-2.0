...
export default function ServicesPage() {
  ...
  const { data: services } = useCollection<ProfessionalService>(servicesCollectionQuery);
  const serviceCount = services?.length || 0;

  return (
    <div className="min-h-screen bg-gradient-to-b from-background to-secondary/20 pt-32">
      {/* Hero Section */}
      <div className="bg-primary text-primary-foreground py-16 text-center">
        <div className="container mx-auto px-4">
            <h1 className="font-headline text-4xl md:text-5xl font-bold mb-4">Professional Services</h1>
            ...
            <div className="flex justify-center items-center gap-4 flex-wrap">
                {professionalServicesFeatures.map(feature => (
                    <Badge 
                      key={feature.title} 
                      variant="secondary" 
                      className="bg-white/20 text-white border-white/30 backdrop-blur-sm text-sm py-1 px-3"
                    >
        ...
        <div className="container mx-auto px-4">
          <div className="text-center mb-12">
            <h2 className="font-headline text-3xl md:text-4xl font-bold text-foreground">
              Choose from {serviceCount} Professional Services
            </h2>
            <p className="mt-2 text-muted-foreground">
              Click on any service to view detailed information, pricing, and apply directly
            </p>
          </div>
          <ServicesList />
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 text-center">
                {trustBadges.map(badge => (
                    <div key={badge.title} className="flex flex-col items-center">
                        ...
                        <h3 className="text-xl font-semibold text-foreground mb-2">{badge.title}</h3>
